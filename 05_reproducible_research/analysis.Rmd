...
```{r}
# adjust columns
# a <- storm %>% select(ends_with("EXP"))
# a$PROPDMGEXP %>% unique()
# a$CROPDMGEXP %>% unique()
# a %>% filter(str_detect(str_to_lower(PROPDMGEXP), "k|m|b"))
# a %>% filter(!str_detect(str_to_lower(PROPDMGEXP), "k|m|b"))
# a %>% filter(is.na(PROPDMGEXP))
# nrow(a)
# 
# storm %>% transmute(prop = if_else(str_detect(str_to_lower(PROPDMGEXP), "k"), "1000", PROPDMGEXP))
# 
# mutate(a, pr = as.numeric(a$prop) * storm$PROPDMG)

for (i in seq_len(nrow(storm))) {
    ex <- NA
    if (str_to_lower(storm[i, ]$PROPDMGEXP) == "k") {ex <- 3}
    else if (str_to_lower(storm[i, ]$PROPDMGEXP) == "m") {ex <- 6}
    else if (str_to_lower(storm[i, ]$PROPDMGEXP) == "b") {ex <- 9}
    else if (str_to_lower(storm[i, ]$PROPDMGEXP) %>%
             as.numeric() %>%
             is.na() %>%
             isFALSE()) {ex <- as.numeric(storm[i, ]$PROPDMGEXP)}
    storm[i, ]$PROPDMGEXP <- ex
}
```
