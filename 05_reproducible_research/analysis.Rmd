...
ggplot(storm, aes(x=ev, y=PROPDMG, fill=ev)) +
                        mutate(e = factor(ev, ev)) %>% 
    theme(axis.text.x = element_text(angle = 90)) +
    scale_x_discrete(limits = PROPDMG) +
    geom_bar(stat="identity")


panel_plot1 <- grid.arrange(plot_total_propdmg,
                            plot_total_cropdmg,
...
g <- ggplot(storm, aes(ev)) + theme(axis.text.x = element_text(angle = 90))

g + geom_bar(aes(weight = FATALITIES))

ggplot(storm) + geom_bar(aes(event, weight = economy, fill = ev)) + theme(axis.text.x = element_text(angle = 90)) + scale_x_discrete(limits = head(res$ev, n = 10))
